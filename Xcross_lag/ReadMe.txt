================================================================================
    MFC 程式庫 : Xcross_lag 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 Xcross_lag 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 Xcross_lag 應用程式的檔案。
...
Xcross_lag.h
    ...
    並宣告 CXcross_lagApp 應用程式類別。

Xcross_lag.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CXcross_lagApp。
...
Xcross_lagDlg.h、Xcross_lagDlg.cpp - 對話方塊
    這些檔案包含您的 CXcross_lagDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 Xcross_lag.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 Xcross_lag.pch 的先行編譯標頭 (PCH) 檔，
